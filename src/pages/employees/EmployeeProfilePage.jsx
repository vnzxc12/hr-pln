...
export const EmployeeProfilePage = () => {
  ...
  // Quick Photo Change Handler with Canvas Compression & Cloud Sync
    ...
    if (file) {
      ...
      reader.onload = (event) => {
        ...
        img.onload = () => {
          ...
          dataService.updateEmployee(employee.id, { profile_photo: compressed }, currentUser);
  ...
  const handleDownloadDocument = (doc) => {
    if (!doc || !doc.file_path) {
      alert('Document file path is unavailable.');
      return;
    }
    ...
    <div className="space-y-6 pb-16">
                    ...
                    <div className="pt-2 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between text-xs">
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <button
                          ...
                          className="px-2.5 py-1 rounded-lg bg-emerald-50 dark:bg-emerald-950/50 hover:bg-emerald-100 dark:hover:bg-emerald-900 text-emerald-700 dark:text-emerald-300 text-xs font-semibold flex items-center gap-1 transition-colors"
      ...
      >
        <div className="space-y-4">
          {/* Top Control Bar */}
          <div className="flex flex-wrap items-center justify-between gap-3 p-3 rounded-2xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-xs">
            <div className="flex items-center gap-2">
              <span className="font-semibold text-slate-700 dark:text-slate-300">File:</span>
              <span className="font-mono text-slate-900 dark:text-white truncate max-w-xs">{previewDoc?.file_name}</span>
              {previewDoc?.file_size && (
                <span className="px-2 py-0.5 rounded-full bg-slate-200 dark:bg-slate-800 text-[11px] text-slate-600 dark:text-slate-400 font-mono">
            ...
            <div className="flex items-center gap-2">
              ...
              {(previewDoc?.file_type?.startsWith('image/') || previewDoc?.file_path?.startsWith('data:image') || previewDoc?.file_path?.includes('unsplash')) && (
                <div className="flex items-center gap-1 bg-white dark:bg-slate-800 p-1 rounded-xl border border-slate-200 dark:border-slate-700">
                  <button
                    ...
                    className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 transition-colors"
                    title="Zoom Out"
                  >
                    <ZoomOut className="w-4 h-4" />
                  </button>
                  <span className="font-mono text-[11px] font-bold px-1.5 min-w-12 text-center text-slate-800 dark:text-slate-200">
                  ...
                  <button
                    ...
                    className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 ml-1 border-l border-slate-200 dark:border-slate-700 transition-colors"
              ...
              <button
                ...
                className="px-3.5 py-1.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold flex items-center gap-1.5 transition-colors shadow-xs cursor-pointer"
          ...
          {/* Large High-Res Content Viewport */}
          <div className="p-4 rounded-2xl bg-slate-950/5 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 min-h-[50vh] max-h-[75vh] overflow-auto flex items-center justify-center relative">
            {previewDoc?.file_type?.startsWith('image/') || previewDoc?.file_path?.startsWith('data:image') || previewDoc?.file_path?.includes('unsplash') ? (
              <div className="overflow-auto flex items-center justify-center p-4 min-w-full min-h-full">
                <img
                  src={previewDoc.file_path}
                  alt={previewDoc.document_name}
                  style={{ transform: `scale(${previewZoom})`, transformOrigin: 'center center' }}
                  className="max-w-full max-h-[70vh] rounded-xl object-contain shadow-2xl transition-transform duration-150"
                />
              </div>
            ) : previewDoc?.file_type?.includes('pdf') || previewDoc?.file_name?.toLowerCase().endsWith('.pdf') || previewDoc?.file_path?.startsWith('data:application/pdf') ? (
              <iframe
                src={previewDoc.file_path}
                title={previewDoc.document_name}
                className="w-full h-[72vh] rounded-xl border border-slate-300 dark:border-slate-700 bg-white"
              />
            ) : (
              <div className="text-center p-12 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm max-w-md mx-auto">
                <div className="w-20 h-20 rounded-3xl bg-emerald-50 dark:bg-emerald-950/60 text-emerald-600 dark:text-emerald-400 flex items-center justify-center mx-auto mb-4 border border-emerald-200 dark:border-emerald-800">
                  <FileText className="w-10 h-10" />
                </div>
                <h4 className="font-bold text-slate-900 dark:text-white text-base">{previewDoc?.document_name}</h4>
                <p className="text-xs text-slate-500 font-mono mt-1">{previewDoc?.file_name}</p>
                <p className="text-xs text-slate-400 mt-2">Click below to download and view this document on your device.</p>
                <button
                  onClick={() => handleDownloadDocument(previewDoc)}
                  className="mt-6 px-6 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-bold inline-flex items-center gap-2 shadow-md transition-colors cursor-pointer"
                >
                  <Download className="w-4 h-4" />
          ...
          <div className="flex items-center justify-between pt-2 border-t border-slate-100 dark:border-slate-800">
            <span className="text-xs text-slate-400">
              Uploaded by {previewDoc?.uploader_name || 'HR Administrator'} on {previewDoc?.created_at ? new Date(previewDoc.created_at).toLocaleDateString() : 'Recent'}
            </span>
            <div className="flex items-center gap-2">
              <button
                onClick={() => handleDownloadDocument(previewDoc)}
